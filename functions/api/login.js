...
export async function onRequestPost({request,env}){
  try{
    ...
    return json({user:{username:u.username,role:env.ADMIN_USERNAME&&u.username.toLowerCase()===String(env.ADMIN_USERNAME).trim().toLowerCase()?'admin':'user'},ok:true},200,{headers:{'Set-Cookie':`shturman_session=${s.token}; Max-Age=2592000; Path=/; HttpOnly; Secure; SameSite=Lax`}})
